...
export default function CameraView() {
  ...
  return (
    <Section title="Live Camera Preview">
      {loading && !error && (
        <div className="bg-base-200 rounded-lg p-10 text-center my-3">
      ...
      {error && (
        <Alert variant="warning">
          <p className="font-medium mb-1">Camera Stream Issue</p>
          <p className="text-xs mb-3">{error}</p>
          <Button variant="secondary" onClick={retryStream}>
            Retry Stream
          </Button>
        </Alert>
      )}
